...
function makeSound() {
  const audio = new Audio("./sounds/click-21156.mp3");
  audio.play();
}
...
function keyAnimation(key) {
  document.querySelectorAll(".key").forEach(button => {
    if (button.textContent.toLowerCase() === key.toLowerCase() || 
        (key === " " && button.textContent === "Space")) {
...
function handleTyping(key) {
  ...
  if (isLetter) {
    if (currentLetter) {
      ...
      if (key !== expected) {
        const wrongAudio = new Audio("./sounds/mixkit-click-error-1110.wav");
        wrongAudio.play();
        document.querySelectorAll(".key").forEach(button => {
          if (button.textContent.toLowerCase() === key.toLowerCase() || 
              (key === " " && button.textContent === "Space")) {
